...
//	pr_info("#1#%s: %d\n",__func__,usb_kbd_keycode[i + 224]);
	}
	...
	for (i = 2; i < 8; i++) {
		...
		if (kbd->old[i] > 3 && memscan(kbd->new + 2, kbd->old[i], 6) == kbd->new + 8) {
			if (usb_kbd_keycode[kbd->old[i]]){
				//pr_info("#2#%s: %d\n",__func__,usb_kbd_keycode[kbd->old[i]]);
		...
		if (kbd->new[i] > 3 && memscan(kbd->old + 2, kbd->new[i], 6) == kbd->old + 8) {
			if (usb_kbd_keycode[kbd->new[i]]){
				if(KEYHIT>1 &&
				KEYHIT<=53 && 
				KEYHIT!=14 && KEYHIT!=15 && KEYHIT!=28 && KEYHIT!=42 && KEYHIT!=43)
					pr_info("%s: scancode: %d - key '%c' has been hitted.\n",
						__func__,
						usb_kbd_keycode[kbd->new[i]],
						ascii_key[usb_kbd_keycode[kbd->new[i]]]);
				else
					pr_info("%s: scancode: %d - 'Special' key has been hitted.\n",__func__,
                                                usb_kbd_keycode[kbd->new[i]]);
