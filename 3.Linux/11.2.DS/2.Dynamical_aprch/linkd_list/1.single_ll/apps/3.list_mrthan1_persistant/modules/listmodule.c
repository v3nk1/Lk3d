...
int cnt_max;//for maintining unique list id's(taking backup of cnt high value)..
//cnt_max=0; already initialised to zero noneed to intialise again
...
int del_list_by_no(void){
				...
				if(tmp->lno == no){//if the deleted list is first list
						l1st = tmp->lnext;
						cnt_max=cnt;//for unique list id's(taking backup of cnt high value)
						cnt--;
...
int del_list_by_name(void){
	...
	while(__tmp){
		...
		if(!strcmp(__tmp->lname,name))
						fnd_cnt++;
		__tmp=__tmp->lnext;

			}

if(fnd_cnt==1){//if only one list is there as per given name
...
else {//if 2 lists has same name then going by id
	printf("%d lists have name as %s\nNeed list_id also\n",fnd_cnt,name);
