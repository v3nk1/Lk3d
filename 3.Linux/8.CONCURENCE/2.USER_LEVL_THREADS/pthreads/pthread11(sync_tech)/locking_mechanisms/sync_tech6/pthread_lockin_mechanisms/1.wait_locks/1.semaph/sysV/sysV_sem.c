...
void *Func1(void *arg){						/* 1)say thread1 has exectuted first */

#ifndef RACECOND
	printf("tid1: Semaphore trying to lock..\n");
	semop(semid,&s_lock,1);					/* 2)semaphore value=0(since subtracted by 1) */
	...
	semop(semid,&s_ulock,1);				 /* 4)semaphore value=1(since added by 1) */
...
void *Func2(void *arg){
	...
	semop(semid,&s_lock,1);					/* 3)Here blocked., becoz val will go negative 
								 * it'll b blocked until sem_val added by 1
								 * ie unblocked after 4 executed.
								 */
...
main(){
	...
	pthread_t tid1,tid2;
	semid = semget(555,3,IPC_CREAT|0666);
	semid1 = semget(555,3,IPC_CREAT|0666);
	arg.val=1;						/* semphore val */
