//Here in this scenario we will see how to block a signal while a another handler is in execution, using sigprocmask API. How to delete a signal registered for blocking.(here its not possible, we hav to pass SIG_SETMASK | SIG_BLOCK )we wil see it next program;
...
void sig_int(int signo){
	...
	printf("caught signal %d (%s)",signo,strsignal(signo));
...
//	alarm(5);// it will execute, becoz we ain't blocked it yet.// becoz of default behaviour of evry signal after it reminds alarm it'll terminate the process. if u dnt want that behaviour then define ur own handler for alarm and register it by using sigaction/signal API.
	...
	sleep(15);    //here if u press ctrl+c and ctrl+\ it will block those signals but BLOCK always keeps those in a que remember that. but here not..........
