...
void sigHandler(int no){
	...
	sleep(5);//here sleep is must and shud becoz to make it as above under lined heading..
...
/* SO HERE WE ARE BLOCKING TWO SIGNALS WHILE A SIGNAL HANDLER IS IN EXECUTION*/
	sigaddset(&obj.sa_mask,SIGQUIT);  // through this fuction we are pushing one by one signal numbrs to mask(assume its like array) of type sigset_t defined in signal.h header file. 
...
`````````````````````````````````````
	1) Declare a object of type sigset_t, which is a predifined in signal.h header file, ie., we are just creating its instance.
	like: sigset_t mask;
