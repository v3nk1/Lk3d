...
/* ethtool ops */
static struct ethtool_ops drvioctl_ops = {
...
static void napi_get_drvinfo(struct net_device *dev,struct ethtool_drvinfo *info)
{
        struct drv_private *np = netdev_priv(dev);
...
/* func declerations */
static int drv_up(struct net_device *dev);
static int napi_poll(struct napi_struct *napi, int budget);
static int start_xmit (struct sk_buff *skb, struct net_device *dev);
static irqreturn_t int_handler (int irq, void *dev_instance);
static int drv_down (struct net_device *dev);
static struct net_device_stats *napi_get_stats (struct net_device *dev);

/* netdev ops */
struct net_device_ops netdrv_ops  __read_mostly = {
        .ndo_open               = drv_up,
        .ndo_stop               = drv_down,
...
static void napi_controller_start (struct net_device *dev)
{
	...
	/* lock Config[01234] and BMCR register writes */
		...
		writel(0, np->iobase + RxMissed);
        	...
        	writew(napi_intr_mask,np->iobase+IMR);    // now enables ISR
}
...
/**
 * drv_up - up a net-interface
 *
 * drv_up get's invoked
...
static int drv_up (struct net_device *dev)
{
	...
	/* register ISR */
        rc = request_irq (np->irq, int_handler, 
 ...
 *
 */
static void napi_isr_ack(struct drv_private *np)
{
        u16 				status;
 ...
 * 	-  calls napi_isr_ack
 */
static int napi_rx(struct net_device *dev, struct drv_private *np,
                      						int budget)
{
 ...
 * 	-  if ok does the polling (napi_rx)
...
static int napi_poll(struct napi_struct *napi, int budget)
{
	struct drv_private 		*np = container_of(napi, struct napi_private, napi);
...
/**
 * start_xmit - starts packet tx'mission
 *
 ...
 * after sending pkt tx'ssion device should trigger an interrupt...
 * sothat ISR will invoked..
 *
 * Responsibilities :
...
static int start_xmit (struct sk_buff *skb, struct net_device *dev)
{
	...
	/* print the egress queued pkt details */
        if (netif_msg_tx_queued(np))
                printk (KERN_INFO "%s: start_xmit() egress queued pkt size %u" 
				" to slot %d.\n", dev->name, len, entry);
...
static void napi_tx_interrupt (struct net_device *dev, struct drv_private *np)
{
...
/**
 * int_handler - ISR (Interrupt Service Routine) handler
 *
 * handler will called for triggered interrupt during 
 * before pkt reception & after pkt transmission
 *
 * Responsibilities :
 * 	-  get the isr
...
/**
 * drv_down - down a net-interface
 *
 * drv_down get's invoked
 ...
 *     PCI initialization
 * 	-  enable device
 * 	-  discover the resources ie base addr IO(address space)
 * 	-  reserve access to address region
 * 	-  Map address region to kernel address space
 * 	-  Enable bus mastering mode
 *     Register Driver with network subsystem 
                                       ...
                                       const struct pci_device_id *ent)
{
	...
	/* allocates net_device object */
        dev = alloc_etherdev(sizeof(struct drv_private));
        if (!dev) {
                rc = -ENOMEM;
	...
	dev->netdev_ops  	= &netdrv_ops;
	dev->ethtool_ops 	= &drvioctl_ops;
	dev->irq 		= pdev->irq;
...
/**
 * drv_exit - Un-probeing for device Not-found
 *
 * SW Uninitialization :
