...
char smname[20],cmname[20];//,tmp[20];
...
//	sync();syncfs(sfd);sync();
	...
	fflush(stdout);
	scanf(" %s",sbuff.sndr);
	sbuff.attr.sndr_sz=strlen(sbuff.sndr)>18 ? 19 :strlen(sbuff.sndr)+1;//including NUL
	sbuff.sndr[19]='\0';
//	strcpy(sbuff.sndr,tname);
	...
	fflush(stdout);
	scanf(" %[^\n]",sbuff.subjct);
	sbuff.attr.sub_sz= strlen(sbuff.subjct)>79 ? 80 : strlen(sbuff.subjct)+1;//including NUL
	sbuff.subjct[80]='\0';
//	strcpy(sbuff.subjct,tsub);
	...
	sbuff.attr.msg_sz= strlen(sbuff.msg)>511 ? 512 : strlen(sbuff.msg)+1;//including NUL
	sbuff.msg[512]='\0';			//if given msg > 512 then no null terminated. its done explicitly.
...
//	attr_t attr;
	for(k=0;k<snd;k++){
//		sync();syncfs(sfd);sync();
		recv(sfd,&sbuff,sizeof(sbuff),0);		//getting back, sizes from server.

//	        recv(sfd,,attr.sndr_sz/*sizeof(tname)*/,0);	//Here sizeof(tname) will read more char than 										required. but those char will discared.
        	printf("\nSENDER : %s@%s\n\n",sbuff.sndr,cmname);
//		sync();syncfs(sfd);sync();
