	...
	switch(fork()){
		...
		//		syncfs(cfd);sync();syncfs(sfd);sync();
				...
				for(i=0;i<msg;i++){
					cbuff.attr=battr[i];
//printf("%s\n",buff+snd);
					//strncpy(buff+snd,cbuff.sndr,battr[i].sndr_sz);
					strcpy(cbuff.sndr,buff+snd);
					snd+=battr[i].sndr_sz;
//printf("%s\n",buff+snd);
					//strncpy(buff+snd,cbuff.subjct,battr[i].sub_sz);
					strcpy(cbuff.subjct,buff+snd);
					snd+=battr[i].sub_sz;
//printf("%s\n",buff+snd);
					//strncpy(buff+snd,cbuff.msg,battr[i].msg_sz);
					strcpy(cbuff.msg,buff+snd);
					snd+=battr[i].msg_sz;

					send(cfd,&cbuff,sizeof(cbuff),0);
//	printf("%s\n",cbuff.sndr);
//	printf("%s\n",cbuff.subjct);
//	printf("%s\n",cbuff.msg);
