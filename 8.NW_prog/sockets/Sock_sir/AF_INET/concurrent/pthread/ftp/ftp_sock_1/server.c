/*Server*/
//METHOD:1(using normal read/write calls)
	/*
	 * Sending data using pack_t structure 
	 */
...
loop:
	...
	while(1){
		...
		rd = read(fl_fd,snd.data_buf,sizeof(snd.data_buf));
		sync();
		if(rd==0){
...
//			printf("last packet sent\n");
		...
		snd.data_sz=rd;
		sync();
		wr=write(curr_cfd,&snd,sizeof(snd));
//		printf("packet snd\n");
		sync();/* Or use syncfs(); */
