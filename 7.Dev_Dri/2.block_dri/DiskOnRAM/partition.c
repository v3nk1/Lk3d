...
{
    pr_info("%s(%d): disk_start: %p\n",__func__,__LINE__,disk);
	memset(disk, 0x0, MBR_SIZE);
	*(unsigned long *)(disk + MBR_DISK_SIGNATURE_OFFSET) = 0x36E5756D;
    pr_info("%s(%d): disk+sig: %p\n",__func__,__LINE__,disk + MBR_DISK_SIGNATURE_OFFSET);
	memcpy(disk + PARTITION_TABLE_OFFSET, &def_part_table, PARTITION_TABLE_SIZE);
    pr_info("%s(%d): disk+ptbl: %p\n",__func__,__LINE__,disk + PARTITION_TABLE_OFFSET);
	*(unsigned short *)(disk + MBR_SIGNATURE_OFFSET) = MBR_SIGNATURE;
    pr_info("%s(%d): disk+aa55: %p\n",__func__,__LINE__,disk + MBR_SIGNATURE_OFFSET);
}
static void copy_br(u8 *disk, int start_cylinder, const PartTable *part_table)
{
	disk += (start_cylinder * 32 /* sectors / cyl */ * SECTOR_SIZE);
    pr_info("%s(%d): disk+br: %p\n",__func__,__LINE__,disk);
	...
	*(unsigned short *)(disk + BR_SIGNATURE_OFFSET) = BR_SIGNATURE;
    pr_info("%s(%d): disk+aa55: %p\n",__func__,__LINE__,disk + BR_SIGNATURE_OFFSET);
}
