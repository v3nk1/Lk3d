...
/*
 * esc bspc tab ent spc caps f1->f12
    1   14   15  28  57  58  59 to 88

 * 1 to =  q to ]  a to '  ~   z to / 
   2 to13  16to27  30to40  41  44to53
 */
			...
			if (IS_IT_ASCII)
				pr_info
				    ("%s: scancode: %d - key '%c' has been hitted.\n",
				     __func__, KEYHIT, ascii_key[KEYHIT]);
			else
				pr_info
			 ...
			 const struct usb_device_id *id)
{
	...
	/*      
	 * Store ref to usb_dev in the private object
	 */
	kbd->usbdev = dev;
...
static int __init usb_kbd_init(void)
{
	...
	if (ret == 0)
		pr_info("%s: usbkbd: Registered with USB Core\n",__func__);
	else
